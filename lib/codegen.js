...
var getViewForSwagger2 = function(opts, type){
    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, m){
            ...
            _.map(responses, function(response, statusCode) {
              if (!_.isObject(response.schema)) {
                  return
              }
              
              if (_.isString(response.schema.$ref)) {
                var segments = response.schema.$ref.split('/');
                response = swagger.definitions[segments.length === 1 ? segments[0] : segments[2]]
              } else if (response.schema.type === 'array' && _.isString(response.schema.items.$ref)) {
              ...
              if (statusCode >= 200 && statusCode < 300) {
                method.successType = response
              }

              if (statusCode >= 400) {
                method.failType = response
              }
            })
